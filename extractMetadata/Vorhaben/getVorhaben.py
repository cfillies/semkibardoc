...
def erstellenVorhabenDict(targetDir, vorhabenOntologie, methode, docxVorhanden=True):
    # Extrahieren der Vorhaben zur Zuordnung der Dateien: gibt ein Verzeichnis mit den aufgelisteten Vorhaben aus
    ...
    for lowDir in lowest_dirs:
        ...
        #try:
            ...
            while (higherDir in subdirectories) and (higherDir not in allVorhDic.keys()):
                ...
                vorhabenHigherDict = extrahierenAlleVorhaben(higherDir, vorhabenOntologie, methode, docxVorhanden)
    ...
    for key in a.keys():
        for innerKey in a[key].keys():
            ...
            if 'vorhaben' not in b[key][innerKey].keys():
                upd_dict = {'vorhaben' : {vorhaben: {'files': [filename], 'datum': sorted(daten, reverse = True), 'preproc': prepVorhaben, 'oberbegriffe': oberbegriffe}}}
                b[key][innerKey].update(upd_dict)
            else:
                if vorhaben not in b[key][innerKey]['vorhaben'].keys():
                    upd_dict = {vorhaben: {'files': [filename], 'datum': sorted(daten, reverse = True), 'preproc': prepVorhaben, 'oberbegriffe': oberbegriffe}}
...
def entfernenWennFürVorhabenGleichhueltig(listWords):
    ...
    for word in listWords:
        ...
        newword = re.sub(r"antrag|anlage|genehmigung|anlage|bauvorhaben|denkmalschutz|anhören|versagen|bescheid|stellungnahme|protokoll", "", word)
...
def explizitesVorhaben(text, moeglVorh, score):
    ...
    if len(tempVorh) > 2:
        ...
        if tempVorh not in moeglVorh:            
            # Vorhaben ist in der Liste für mögliche Vorhaben dieser Ordner vorhanden (sollte immer so sein)  
            forTextFile += ' --> Möglicher Bug: Vorhaben war NICHT in der Liste für mögliche Vorhaben dieses Ordners vorhanden: ' + tempVorh + '\n'
...
#############################################################
### Functions to get the matching Vorhaben based on a "OrdnerStruktur" (as in "OrdnerStrukturTreptow") file instead of
### actually going recursively from one directory to the next
...
def loadVorhabenDict_OROS(ordnerStruktur, dateidir, directories, vorhOntologie, methode):
    ...
    if not vorhabenDict_path.is_file():     
        ...
        allVorhDic = erstellenVorhabenDict_OROS(ordnerStruktur, dateidir, directories, vorhOntologie, methode)
...
def erstellenVorhabenDict_OROS(ordnerStruktur, dateidir, subdirectories, vorhOntologie, methode):
    ...
    for lowDir in lowest_dirs:
        ...
        #try:
            ...
            vorhabenDict = extrahierenAlleVorhaben_OROS(files, dateidir, lowDir, vorhOntologie, methode)
            ...
            while (higherDir in subdirectories) and (higherDir not in allVorhDic.keys()):
                ...
                vorhabenHigherDict = extrahierenAlleVorhaben_OROS(files_higher, dateidir, higherDir, vorhOntologie, methode)

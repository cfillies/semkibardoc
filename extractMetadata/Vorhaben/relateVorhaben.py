...
class initVorhaben:
    def __init__(self, outermost_dir, methode):
        ...
        self.vorhDict, self.subdirs = getVorhaben.loadVorhabenDict(self.vorhDict_dir, self.vorhOntologie, self.methode)
...
class initVorhaben_OROS:
    def __init__(self, ordnerStruktur, dateidir, directories,  methode):
        ...
        self.vorhOntologie = getVorhaben.extrahierenOntologie()
        self.methode = methode
        self.vorhDict, self.subdirs = getVorhaben.loadVorhabenDict_OROS(ordnerStruktur, dateidir, directories, self.vorhOntologie, self.methode)
        self.nlp = spacy.load('de_core_news_lg')
...
def vorhaben(metadata, directories, ordnerStruktur, methode):
    ...
    moeglVorhList, moeglOrtList = getVorhabenListFuerDirectory(target_dir, vorhVars.vorhDict, vorhVars.subdirs)
...
def getVorhabenBestMatch(inhalt, adressenDict, datenDateiAll, moeglVorhList, moeglOrt, allVorhDict, vorhOntol, pfad, dateiname):
    
    score = 11
    vorhabenBestMatch, score, forTextFile = getVorhaben.explizitesVorhaben(inhalt, moeglVorhList, score)
    ...
    if not vorhabenBestMatch:
        ...
        dtMtSort, wtMtSort, obMtSort, adMt = getVorhabenMatches(inhalt, vorhOntol, allVorhDict, moeglOrt, adressenDict, datenDatei)

        vorhabenBestMatch, score = matchbewertungScore(vorhabenBestMatch, dtMtSort, wtMtSort, obMtSort, adMt, score, moeglVorhList)

        forTextFile += "\nMögliche Vorhaben:\n" + str(moeglVorhList) + "\nMögliche Orte:\n" + str(moeglOrt) + "\n"
        forTextFile += "           " + str(dtMtSort) + "\n" + "           " + str(wtMtSort) + "\n" + "           " + str(obMtSort) + "\n" + "\n" + "           " + str(adMt) + "\n"
        forTextFile += "\nVorhaben, score (0 = höchst, 11 = niedrigst):\n           " + str(vorhabenBestMatch) + "," + str(score) + "\n\n\n ------------ \n"
    else:
...
def matchbewertungScore(vorhabenBestMatch, dtMtSort, wtMtSort, obMtSort, adMt, score, moeglVorhList):
    ...
    Herangehensweise:
    1. Das Vorhaben mit dem kleinsten dt (dtMtSort[0] Datumunterschied zw. Vorhaben und Datei) wird bevorzugt.
    2. Scoring wird aufgestellt:
       - Wenn dtMtSort nicht leer ist:                                                               +1 Punkt
       - Wenn dtMtSort[0] kleiner ist als 90 Tage (3 Monate):                                        +4 Punkte; Wenn dtMtSort[0] kleiner ist als 365 Tage (1 Jahr): +2 Punkt
       - Wenn dtMtSort[0] in wtMtSort UND obMtSort vorhanden ist:                                    +4 Punkte, Wenn dtMtSort[0] in wtMtSort ODER obMtSort vorhanden ist: +3 Punkte
       - Wenn der Ort, z.B. Akazienhof, zu den aufgelisteten Vorhaben passt:                         +2 Punkt

       - Wenn dtMtSort leer ist, aber wtMtSort gleich obMtSort ist (& beide nicht leer):             +3 Punkt
            ...
            for vorhkey in allVorhDict[key][innerkey]['vorhaben'].keys():
                ...
                if allVorhDict[key][innerkey]['vorhaben'][vorhkey]['datum']:
                    if type(allVorhDict[key][innerkey]['vorhaben'][vorhkey]['datum'][0]) == str:
                        tempTime = helpers.convertstring2date(allVorhDict[key][innerkey]['vorhaben'][vorhkey]['datum'][0])
                    else:

...
# #print(sys.path[0])
import extract.extractText as eText
...
def filename_divider(filename):
    ...
    fnameNew = fname.replace(".", " ").replace("_", " ").replace("-", " ")\
        .replace("'", " ").replace(",", " ")
    return fnameNew
...
def remManContent(inhalt):
    ...
    if searchAuflage:
        startInhalt = searchAuflage.span()[
            1]  # [1] damit Auflafe auch im Match berücksichtigt wird.
...
def postProcessMatch(matchList):
    for file, val in list(matchList.items()):
        if len(val.keys()) > 1:
            ...
            if len(val.keys()) == 3 and (
                    'Genehmigung' and 'Versagung' and 'Anhörung') in val.keys():
                del matchList[file]['Genehmigung']
                del matchList[file]['Versagung']
...
def resultSummary(matchList, noMatchList, wrongFormatList, errorList, filePath):
    i = 0
    points = {'Gesamt-Zuordnung': 0, 'Informelles-Format': len(wrongFormatList),
              'Keine-Zuordnung': len(noMatchList.keys()), 'Fehler': len(errorList),
              'Genehmigung': 0, 'Versagung': 0, 'Anhörung': 0, 'Antrag': 0, 'Anfrage': 0,
              'Nachforderung': 0, 'Bauverfahren': 0, 'Eingang': 0}
    ...
    #
    resultDict.setdefault('Gesamt-Zuordnung', {})[filePath] = matchList
    #
    resultDict.setdefault('Keine-Zuordnung', {})[filePath] = noMatchList
    #
    resultDict.setdefault('Informelles-Format', {})[filePath] = wrongFormatList
    #
    resultDict.setdefault('Fehler', {})[filePath] = errorList
    #
    return resultDict


def mainProcess(files, filePath, considerDocName, methode, docxVorhanden):
    wrongFormatList = []
    matchList = {}
    noMatchList = {}
    errorList = []
    for i in range(0, len(files)):
        try:
            ...
            """
            pages=eText.getPageNumber(filePath,files[i],methode, docxVorhanden)
            ############################################
            if pages<=15:
                inhalt =eText.getInhalt(filePath,files[i],methode, docxVorhanden)
            ...
            if 'FORMCHECKBOX' in inhalt:
                ...
                if pages<=5:
                    ...
                    wrongFormatList.append(files[i])
                """
                wrongFormatList.append(
                    files[i])  ## Provisorisch hinzugefügt, da python-tika nicht verwendet
                ## Provisorisch auskommentiert, da python-tika nicht verwendet
                continue
    ...
    errorList = errorList
    ##############################################
    return resultSummary(matchList, noMatchList, wrongFormatList, errorList, filePath)


#########################
class findVorgang:
    def __init__(self, files, path, considerDocName, methode, docxVorhanden):
        if type(files) != list:
            self.files = [files]
        else:
            self.files = files
        ...
        self.docxVorhanden = docxVorhanden
        ###############################################
        ...
        for f in self.files:
            ...
            if f in self.zuordnung[self.filePath].keys():
                vResult = rep(list(self.zuordnung[self.filePath][f].keys()))
            elif f in self.keineZuordnung[self.filePath].keys():
                vResult = 'Keine Kategorie gefunden'
            elif f in self.informellesFormat[self.filePath]:
                vResult = 'Informelles Format'
            elif f in self.fehler[self.filePath]:

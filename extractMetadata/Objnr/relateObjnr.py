...
def objnrByPath(root, files, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, deutsch, behoerdenDict, count, foundObjnrPfad, considerPfadundName = True):
    ...
    if considerPfadundName:
        ...
        if (dictResult_single['pfadtreffer']) and ('exact_match_adresse' in dictResult_single['pfadtreffer']['method']):
                # mindestens eine Objektnummer konnte für den gesamten Ordner durch den Pfad bestimmt werden. Nur noch die Vorhaben der Dateien in diesem Ordner fehlen.             
                count += len(files)
                foundObjnrPfad = True   
    else:
        dictResult_single = {}
...
def objnrByFileName(root: str, file: str, pfadDictFile, parser: str, docxVorhanden, deutsch, denkmale, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, behoerden, foundObjnrPfad, countDateiname, countInhalt, considerPfadundName = True):
    ...
    if foundObjnrPfad == True:
        ...
        if considerPfadundName:
            ...
            dictObjnr, countDateiname, foundObjnrDatei = objnrDurchAdresseInDateiname(file, deutsch, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, behoerden, countDateiname, foundObjnrDatei)
    ...
    if (foundObjnrDatei == False) and (inhaltVorhanden == True):                
        ...
        dictObjnr, countInhalt, foundObjnrDatei = objnrImInhalt(inhalt, denkmale, countInhalt, foundObjnrDatei)


    if (foundObjnrDatei == False) and (inhaltVorhanden == True):  
        # Versuch 3.2.: Adresse(n) aus Datei-Inhalt extrahieren      
        dictObjnr, countInhalt, foundObjnrDatei =  objnrDurchAdresseImInhalt(inhalt, deutsch, behoerden, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, countInhalt, foundObjnrDatei)
...
def objnrDurchAdresseInDateiname(file, deutsch, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, behoerdenDict, count, foundObjnrDatei):
    ...
    if adressenAusDateiName: 
        summaryDictDatei = getObjnr.getObjnrAdresse(adressenAusDateiName, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, behoerdenDict)

    summaryDictDatei, count, foundObjnrDatei = getObjnr.pruefenObExakt(summaryDictDatei, count, foundObjnrDatei)
...
def objnrImInhalt(inhalt, denkmale, count, foundObjnrDatei):
    dictResult, foundObjnrDatei, count = getObjnr.getObjnrDirekt(inhalt, denkmale, count, foundObjnrDatei)
...
def objnrDurchAdresseImInhalt(inhalt, deutsch, behoerden, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, count, foundObjnrDatei):
    ...
    if adressen:
        dictResult_singleInhalt = getObjnr.getObjnrAdresse(adressen, denkmalStrasse, denkmaleAdresse, denkmalHausnr, denkmaleObjNr, denkmalSachbegriff, denkmalName, behoerden)
    ...
    if ('method' in dictResult_singleInhalt):                    
        ...
        for key in dictResult_singleInhalt.keys():
            ...
            if (dictResult_singleInhalt[key]['treffer']['exact_match_adresse']):
                ...
                dictResult_singleInhalt, foundObjnrDatei, keepKey = getObjnr.pruefenGlaubwuerdigkeit(dictResult_singleInhalt, key, 'exact_match_strasse', inhalt, foundObjnrDatei) 

                if (foundObjnrDatei == False) and (dictResult_singleInhalt[key]['treffer']['closest_match']): 
                    dictResult_singleInhalt, foundObjnrDatei, keepKey2 = getObjnr.pruefenGlaubwuerdigkeit(dictResult_singleInhalt, key, 'closest_match', inhalt, foundObjnrDatei)  

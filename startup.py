...
@app.route('/hidafacet', methods=['GET'])
def hidafacet():
    if user == None:
        return redirect(url_for('login'))
    return app.send_static_file('hida.html')
...
@ app.route('/document/<docid>/edit', methods=['GET', 'POST'])
...
@app.route("/showdocuments", methods=['GET'])
def showdocuments():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@app.route("/showdocument", methods=['GET'])
def showdocument():
    if user == None:
        return redirect(url_for('login'))
    if metadatatable in collist:
...
@ app.route("/showhida/<id>", methods=['GET'])
def showhida(id=""):
    collist = mydb.list_collection_names()
    if "hida" in collist:
        hida_col = mydb["hida"]
...
@ app.route("/showtaxo", methods=['GET'])
def showtaxo():
    query = request.args
    vi = []
    if "taxo" in collist:
...
@ app.route("/taxoexcel", methods=['GET'])
def taxoexcel():
    query = request.args
    vi = []
    if "taxo" in collist:
...
@ app.route("/intents/<intent>", methods=['GET'])
def intents(intent=""):
    if user == None:
        return redirect(url_for('login'))
    # uri = os.getenv("MONGO_CONNECTION")
...
@ app.route("/showintents", methods=['GET'])
def showintents():
    if user == None:
        return redirect(url_for('login'))
    vi = {}
...
@ app.route("/intentssexcel", methods=['GET'])
def intentssexcel():
    vi = []
    if "vorhaben_inv" in collist:
        vorhabeninv_col = mydb["vorhaben_inv"]
...
@ app.route("/wordsexcel", methods=['GET'])
def wordsexcel():
    vi = []
    if "vorhaben_inv" in collist:
        vorhabeninv_col = mydb["vorhaben_inv"]
...
@ app.route('/pattern/<id>', methods=['GET'])
def pattern(id):
    if user == None:
        return redirect(url_for('login'))
...
@ app.route("/showpattern", methods=['GET'])
def showpattern():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@ app.route('/pattern/<id>/edit', methods=['GET', 'POST'])
...
@ app.route('/pattern/<id>/delete', methods=['POST'])
...
@ app.route("/showdoctypes", methods=['GET'])
def showdoctypes():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@ app.route("/showbadlist", methods=['GET'])
def showbadlist():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@ app.route('/badlist/<id>', methods=['GET'])
def badlist(id):
    if user == None:
        return redirect(url_for('login'))
...
@ app.route('/badlist/<id>/edit', methods=['GET', 'POST'])
def editbadlist(id):
    if user == None:
        return redirect(url_for('login'))
    item = get_item("badlist", id)
...
@ app.route('/badlist/<id>/delete', methods=('POST',))
...
@ app.route("/showemblist", methods=['GET'])
def showemblist():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@ app.route('/emblist/<id>/edit', methods=['GET', 'POST'])
def editemblist(id):
    if user == None:
        return redirect(url_for('login'))
...
@ app.route("/shownoemblist", methods=['GET'])
def shownoemblist():
    if user == None:
        return redirect(url_for('login'))
    vi = []
...
@ app.route('/noemblist/<id>/edit', methods=['GET', 'POST'])
...
@ app.route("/allshowkeywords", methods=['GET'])
def allshowkeywords():
    if user == None:
        return redirect(url_for('login'))
    query = request.args
...
@ app.route("/showkeywords/<docid>", methods=['GET'])
...
@ app.route('/create_extraction', methods=['GET', 'POST'])
...
@ app.route("/search/resolved2_facets", methods=['GET'])
...
@ app.route("/search/metadata_facets", methods=['POST'])
def metadata_facets():
    if user == None:
        return redirect(url_for('login'))
...
@ app.route("/search/resolved2", methods=['GET'])
def resolved2():
    if user == None:
        return redirect(url_for('login'))
    # pagination
...
@ app.route("/search/hida2_facets", methods=['GET'])
def hida2_facets():
...
@ app.route("/search/hida2", methods=['GET'])
def hida2():
    # pagination
    page = int(request.args.get('page', '0'))
...
@app.route('/excel/qs', methods=['GET'])
def excelqs2():
    if user == None:
        return redirect(url_for('login'))
    col = mydb[metadatatable]
...
@ app.route("/spacy/hasvector", methods=['GET', 'POST'])
def hasvector():
    text = ""
    corpus = spacy_default_corpus
    query = request.args
...
@ app.route("/spacy/mostsimilar", methods=['GET', 'POST'])
def mostsimilar():
    word = ""
    topn = 10
    corpus = spacy_default_corpus
...
@ app.route("/spacy/matchingconcepts", methods=['GET', 'POST'])
...
@ app.route("/spacy/extractintents", methods=['GET', 'POST'])
def extractintents():
    text = ""
    ontology: dict[str, list[str]] = {}
    pattern: list[str] = []
...
@ app.route("/spacy/extractlemmata", methods=['GET', 'POST'])
def extractlemmata():
    text = ""
    corpus = spacy_default_corpus
...
@ app.route("/spacy/displacy", methods=['POST'])
def displacy():
    text: str = ""
    ents: list[any] = []
    catlist, colors = allcategories_and_colors()
...
@ app.route("/metadata/clonecollection", methods=['POST'])
...
@ app.route("/showcopydatabase", methods=['GET', 'POST'])
...
@ app.route("/metadata/clonedatabase", methods=['GET', 'POST'])
...
@ app.route("/metadata/insert_many", methods=['POST'])
def insert_many_srv():
    colname = ""
    dict = {}
    if request.method == 'POST':
...
@ app.route("/metadata/insert_one", methods=['POST'])
def insert_one_srv():
    colname = ""
    dict = {}
    if request.method == 'POST':
...
@ app.route("/metadata/init", methods=['POST'])
def init_database():
    if request.method == 'POST':
        if request.json:
...
@ app.route("/showprojectmetadata", methods=['GET', 'POST'])
...
@ app.route("/metadata/project", methods=['POST'])
...
@ app.route("/showextractmetadata", methods=['GET', 'POST'])
...
#         "isintents": False
# }
@ app.route("/metadata/extract", methods=['POST'])
...
@ app.route("/metadata/getlog", methods=['POST'])
def getlog_metadata():
    topn = 10
...
@ app.route("/metadata/resetlog", methods=['POST'])
...
@ app.route("/showlogdata", methods=['GET', 'POST'])
def show_logdata():
    # if request.method == 'POST':
    #     print(request)
    log = getLog(10)
...
@ app.route("/cancelall", methods=['GET', 'POST'])
def cancelall():
    log: any = getLog(1)
    if log != {}:
        cancel_execution()

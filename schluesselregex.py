...
adresse_alternative = re.compile(
    '[.\w-]{1,20}?(?:\s)?(?:strasse|str.|str|straße|allee|weide|Shof|weg|platz|damm|berg|promenade)\W?(?=\s|$) \d{1}(?:\w{1,2}\.?)?(?:-|/\.?)?(?:\w{1,3}\.?)?',
    re.IGNORECASE)

adresse = re.compile(
    r'[.\w-]{1,20}?(?:\s)?(?:strasse|str.|str|straße|ufer|allee|hof|weide|anlage|gasse|graben|gestell|chaussee|wache|weg|platz|promenade|pforte|pfad|damm|tor|brücke|berg)\s*\W??(?:\s)?\d{1}(?:\w{1,2}\.?)?(?:-|/\.?)?(?:\w{1,3}\.?)?',
    re.IGNORECASE)
# Alternativer regex für `adresse`, der Whitespaces vor Bindestrichen berücksichtigt
# re.compile(r'[.\w-]|\s){2,20}?(?:\s)?(?:strasse|str.|str|straße|ufer|allee|hof|weide|anlage|gasse|graben|gestell|chaussee|wache|weg|platz|promenade|pforte|pfad|damm|tor|brücke|berg)\s*\W??(?:\s)?\d{1}(?:\w{1,2}\.?)?(?:-|/\.?)?(?:\w{1,3}\.?)?'
...
plz = re.compile(
    r'\b(1[0-4]{1}[0-9]{3})\b')  # PLZ in Berlin: zwischen 10001-14330 (https://cebus.net/de/plz-bundesland.htm), hier wird erstmal vereinfacht und 10000-14999 berücksichtigt
objnr = re.compile(r'\b(090[0-9]{5})\b')

behoerde = re.compile(
    '(?:Charlottenburg-Wilmersdorf|Charlottenburg Wilmersdorf|Friedrichshain-Kreuzberg|Friedrichshain Kreuzberg|Lichtenberg|Marzahn-Hellersdorf|Marzahn Hellersdorf|Mitte|Neukölln|Pankow|Reinickendorf|Spandau|Steglitz-Zehlendorf|Steglitz Zehlendorf|Tempelhof-Schöneberg|Tempelhof Schöneberg|Treptow-Köpenick|Treptow Köpenick|Oberste Denkmalschutzbehörde|Landesdenkmalamt)',
    re.IGNORECASE)

adresseUnvollstaendig = re.compile(
    '[.A-Za-z0-9_äÄöÖüÜß-]{1,20}?(?:\s)?(?:straße|str.|str|strasse|ufer|allee|weide|hof|gestell|gasse|graben|gestell|chaussee|wache|weg|platz|promenade|pforte|pfad|damm|tor|brücke|berg)(?!\w)',

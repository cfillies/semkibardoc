...
datum            = re.compile('(?:(?<!\:)(?<!\:\d)[0-9]?\d(?:.)?\s+(?:of\s+)?(?:jan\.?|januar|feb\.?|februar|maer\.?|maerz|mär\.?|märz|apr\.?|april|mai|jun\.?|juni|jul\.?|juli|aug\.?|august|sep\.?|september|okt\.?|oktober|nov\.?|november|dez\.?|dezember)|(?:jan\.?|januar|feb\.?|februar|maer\.?|maerz|mär\.?|märz|apr\.?|april|mai|jun\.?|juni|jul\.?|juli|aug\.?|august|sep\.?|september|okt\.?|oktober|nov\.?|november|dez\.?|dezember)\s+(?<!\:)(?<!\:\d)[0-9]?\d(?:.)?)(?:\,)?\s*(?:\d{4})?|[0-3]?\d[-\./][0-3]?\d[-\./]\d{2,4}', re.IGNORECASE)
...
adresse_alternative = re.compile('[.\w-]{1,20}?(?:\s)?(?:strasse|str.|str|straße|allee|hof|weg|platz|damm|berg|promenade)\W?(?=\s|$) \d{1}(?:\w{1,2}\.?)?(?:-|/\.?)?(?:\w{1,3}\.?)?', re.IGNORECASE)

adresse          = re.compile('[.\w-]{1,20}?(?:\s)?(?:straße|str.|str|straße|ufer|allee|hof|anlage|gasse|graben|chaussee|wache|weg|platz|promenade|pforte|pfad|damm|tor|brücke|berg)\W??(?:\s)?\d{1}(?:\w{1,2}\.?)?(?:-|/\.?)?(?:\w{1,3}\.?)?', re.IGNORECASE)
...
adresseUnvollständig          = re.compile('[.\w-]{1,20}?(?:\s)?(?:straße|str.|str|straße|ufer|allee|hof|anlage|gasse|graben|chaussee|wache|weg|platz|promenade|pforte|pfad|damm|tor|brücke|berg)(?!\w)', re.IGNORECASE)
...
regex_type = {
  ...
  "email"             : email,
  "adresse"           : adresse,
  "adresseUnvollständig"           : adresseUnvollständig,
  "plz"               : plz,
  "objnr"             : objnr,
  "behoerde"             : behoerde
}
